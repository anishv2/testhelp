...
const FAQ = async () => {
  ...
  return (
    <Section className="py-10 sm:py-20 bg-gradient-to-b from-gray-100 to-gray-50">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-col gap-4 lg:min-w-4xl sm:max-w-3xl mx-auto">
            {faqData?.data?.map(({ question, answer }, ind) => (
              <Accordian key={ind} title={question} content={answer} isLast={faqData?.data?.length-1 === ind}/>
            ))}
          </div>
        </div>
      </Container>
